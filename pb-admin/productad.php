...
uses("product","attachment", "tag", "typeoption","area","industry","meta","productad","productadtype");
...
if (isset($_POST['saveorder'])) {
	foreach ($_POST['order'] as $key => $val) {
		//echo $key." ".$val;
		$productad->saveField("`order`",$val,intval($key));
	}
}
if (isset($_GET['do'])) {
	$do = trim($_GET['do']);
...
$conditions[] = "pa.productadtype_id IS NOT NULL";
$joins[] = "LEFT JOIN {$tb_prefix}companies c ON c.id=Product.company_id";
$joins[] = "LEFT JOIN {$tb_prefix}productads pa ON pa.product_id=Product.id";
$amount = $product->findCount($joins, $conditions,"Product.id");
$page->setPagenav($amount);
$fields = "pa.`order` as ad_order,pa.id as ad_id,Product.id,Product.company_id AS CompanyID,c.id AS CID,c.name AS companyname,Product.name AS ProductName,Product.status AS ProductStatus,Product.created,Product.ifcommend as Ifcommend, Product.state as ProductState,Product.picture as ProductPicture ";
$result = $product->findAll($fields, $joins, $conditions,"ad_order",$page->firstcount,$page->displaypg);
if (!empty($result)) {
