...
uses("industry","attachment", "typeoption");
$cache = new Caches();
...
if (isset($_POST['save'])) {
	...
	$image_link = 0;
	$attachment->rename_file = "industry-".$time_stamp;
	if(isset($_POST['id'])){
		$id = intval($_POST['id']);
	}
	if(!empty($id)){
		$attachment->rename_file = "industry-new-".$id;
	}
	if (!empty($_FILES['pic']['name'])) {
		$attachment->upload_process();
		$image_link = $attachment->file_full_url;
	}
	...
	if (isset($_POST['id'])) {
		...
		//picture
		if ($image_link) $vals['picture'] = $image_link;
		//highlight
...
//var_dump($vals);
		...
		if (!empty($names)) {
			foreach ($names as $val) {
				$name = $val;
				if(!empty($name)) $tmp_name[] = "('".$name."','".$_POST['data']['industry']['url']."','".$parent_id."','".$top_parentid."','".$level."','".$_POST['data']['industry']['display_order']."','".$_POST['data']['industry']['industrytype_id']."','".$image_link."')";
			}
