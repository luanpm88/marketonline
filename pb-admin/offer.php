...
if(isset($_GET['do'])){
	...
	if ($do == "search") {
		...
		if(!empty($_GET['item_id'])) {
			$conditions[]= "Trade.id=".intval($_GET['item_id']);
		}
	}
	...
	if ($do=="for_student" && $id) {
		$trade->saveField("for_student", intval($_GET["value"]), intval($id));
		pheader("location:".$_SERVER['HTTP_REFERER']);
	}
}
...
$fields = "Trade.for_student,Trade.area_show,Trade.valid_status,Trade.member_id,m.username,Trade.company_id,Trade.adwords,Trade.highlight,Trade.type_id,Trade.status,Trade.id,Trade.title,Trade.clicked,Trade.if_urgent,Trade.submit_time AS pubdate,Trade.submit_time,Trade.modified,Trade.expire_time AS expdate,Trade.expire_time,Trade.picture as TradePicture,require_point,require_membertype,ip_addr as IP,Trade.if_commend";
...
if (!empty($result)) {
	for($i=0; $i<count($result); $i++){
		...
		$result[$i]['preview_link'] = $trade->url(array("module"=>"offers","id"=>$result[$i]["id"],"title"=>$result[$i]["title"]));
	}
	setvar("Items", $result);
}
