...
$fields = "com.shop_name,Member.space_name,Member.id,username,CONCAT(mf.first_name,mf.last_name) AS NickName,mf.reg_ip,last_ip,points,credits,membergroup_id,Member.status,Member.created AS pubdate,last_login,trusttype_ids,checkout,links.parent_id,rlinks.mm";

//$conditions[] = "(rlinks.mm IS NOT NULL OR links.parent_id IS NOT NULL)";
//$conditions[] = "Member.id = rlinks.parent_id";
$conditions[] = "Member.checkout = 1";


$joins[] = "LEFT JOIN {$tb_prefix}memberfields mf ON Member.id=mf.member_id";
$joins[] = "LEFT JOIN {$tb_prefix}companies com ON Member.id=com.member_id";
$joins[] = "LEFT JOIN (SELECT member_id,MAX(created) as lastcheck_created FROM {$tb_prefix}checkouttransactions GROUP BY member_id) ct ON ct.member_id=Member.id";
...
$result = $member->findAll($fields, $joins, $conditions, "lastcheck_created DESC, Member.created DESC ",$page->firstcount,$page->displaypg);
if (!empty($result)) {
	for($i=0; $i<count($result); $i++){
		...
		$result[$i]["url"] = URL.$result[$i]["space_name"];
		...
		$result[$i]["grand"] = $member->getInfoById($result[$i]["parent"]["parent_id"]);
