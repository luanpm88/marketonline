...
function smarty_block_offer($params, $content, &$smarty, &$repeat) {
	...
	if (!$params['recursive']) {
		$sql = "SELECT t.*,t.title as name,t.title as fulltitle,t.content as fullcontent,c.cache_spacename AS userid FROM {$trade->table_prefix}trades t LEFT JOIN {$trade->table_prefix}companies c ON c.id=t.company_id ".$trade->getCondition()."{$orderby}".$mysql_limit;
	}else{
		$sql = "SELECT t.*,t.content as fullcontent,c.cache_spacename AS userid FROM {$trade->table_prefix}trades t ".$trade->getCondition()." ORDER BY t.display_order ASC,t.id DESC".$mysql_limit;
	}
	...
	if(list($key, $item) = each($smarty->blockvars[$param_count])) {
		...
		$space_controller->setBaseUrlByUserId($item['userid'], "offer");
		$url = $space_controller->rewriteDetail("offer", $item['id']);
		$item['url'] = $url;
