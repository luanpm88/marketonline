...
function smarty_block_companynews($params, $content, &$smarty, &$repeat) {
   ...
   if (class_exists("Space")) {
	...
	if(list($key, $item) = each($smarty->blockvars[$param_count])) {
		...
		$item['created_d'] = date('d', $item['created']);
		$item['created_f'] = date('m', $item['created']);
		$item['created_y'] = date('Y', $item['created']);
		$item['created_time'] = date('h:i a', $item['created']);
