...
(function($) {
	$.fn.emotions = function(settings,more_emotions) {
		var _cfg = {
			handle: 'a',
			dir: ROOT_URL+'templates/default/js/jemotion/emotions/',
			...
			emotions: [
				 ...
				 {syntax: ':oo',title: 'sick',icon: '31.gif'},
				 ...
				 {syntax: ':bd',title: 'thumbs up',icon: '113.gif'},
				 {syntax: 'o|^_^|o',title: 'thumbs up',icon: 'music.gif'},
				 {syntax: '(y)',title: 'like',icon: 'like.png'}
			]
		};
			...
			__load: function(){ // apply emotion
				...
				obj.each(function(){
					...
					$.each(_cfg.emotions,function(iEM,bbcode){
						str = str.replace(new RegExp(funct.__regexp(bbcode.syntax),'ig'),'<span class="emotion_show '+iEM+'"><img rel="'+escape(bbcode.syntax)+'" src="'+_cfg.dir+bbcode.icon+'" title="'+bbcode.title+'" /></span>');
					});
					$(this).html(str);
