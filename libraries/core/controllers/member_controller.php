		...
		if (!empty($_GET['em'])) {
			...
			if (!$result) {
				...
				if ($member_reg_auth == 1) {
					...
					$sended = pb_sendmail(array($email, $member_info['username']), $arrTemplate["_pls_active_your_account_title"]." ".$member_info['username'], "activite");
		...
		if (isset($_POST['do'])) {
			...
			if (!empty($userpass) && !empty($username)) {
				...
				if (!$user_exists) {
					...
					if ($result) {
						flash(array("message"=>"reset_and_login", "box_title"=>"Thiết lập lại mật khẩu thành công", "page_title"=>"Thiết lập lại mật khẩu"), "logging.php");
					}
				}
			}
		} else {
