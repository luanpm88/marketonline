		...
		if (!empty($result)) {
			for($i=0; $i<count($result); $i++){
				...
				if($result[$i]["from_member_id"] != $pb_userinfo["pb_userid"])
				{
					...
					{
						$result[$i]["company_logo"] = "1".pb_get_attachmenturl($result[$i]["picture"], '', 'smaller');
					}
					else
					{						
						if($mem["photo"])
						{
							$result[$i]["company_logo"] = "2".pb_get_attachmenturl($mem["photo"], '', 'small');
						}
						else
						{
				...
				else
				{
					...
					{
						$result[$i]["company_logo"] = "3".pb_get_attachmenturl($result[$i]["picture_2"], '', 'smaller');
					}
					else
					{						
						if($mem["photo"])
						{
							$result[$i]["company_logo"] = "4".pb_get_attachmenturl($mem["photo"], '', 'small');
						}
						else
						{
							$result[$i]["company_logo"] = URL."templates/default/image/usericon_big.png";
