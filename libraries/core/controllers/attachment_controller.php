...
class Attachment extends PbController {
    ...
    var $upload_form_field = 'userfile';
   	var $allowed_file_ext = array('.jpg', '.jpeg', '.gif', '.png', '.bmp', '.swf', '.flv', '.mp4');
...
//		$this->upload_url = str_replace(array(DS, "\\", "\'"), "/", $this->upload_dir).'/'; 	
 	...
 	function upload_process($type_id = '0')
 	{
		...
		$this->upload_url = str_replace(array(DS, "\\", "\'"), "/", $this->upload_dir).'/';
 		if (isset($_FILES) && $_FILES[$this->upload_form_field]['size']>0) {
			...
			$logdata['source_module'] = "imageupload";
			$logdata['description'] = $_FILES[$this->upload_form_field]['size']."-".$_FILES[$this->upload_form_field]['name'];
			$log->Add($logdata);
	        ...
	        if ($this->is_image) {
		        ...
		        if($this->if_thumb){
			        require_once(LIB_PATH. "thumb.class.php");
			        if ($this->if_orignal) {
				...
				{
					$this->resizeImage($this->out_file_full_path, 300, 300, "square");
			...
			if($this->if_banner)
			{
				...
				$image = WideImage::load($this->out_file_full_path);
				$resized = $image->resize(907, 339, 'outside');
				list($widthz, $heightz) = @getimagesize($this->out_file_full_path);
				
				$new_pos_top = 0;
				$new_pos_left = 0;
				$tile_goc = $widthz/$heightz;
				$tile_sauresize = 907/339;
				
				if($tile_goc < $tile_sauresize)
				{
					$new_pos_top = ($heightz/($widthz/907)-339)/2;
				}
				else
				{
					$new_pos_left = ($widthz/($heightz/339)-907)/2;
				}
				
				$cropped = $resized->crop($new_pos_left, $new_pos_top, 907, 339);
				$cropped->saveToFile($this->out_file_full_path.".banner.jpg");
			}
	...
	function upload_editor_process($type_id = '0')
 	{
 		$attach_info = array();
 		//init moved here
		if(empty($this->upload_dir)) $this->upload_dir = gmdate("Y").DS.gmdate("m").DS.gmdate("d");
	...
	function resizeImage($source, $param_w, $param_h, $tail)
	{		
		//echo $width;
		//echo "sdfsdfsdfsdf";
		require_once(LIB_PATH. "wideimage/WideImage.php");
		...
		list($widthz, $heightz) = @getimagesize($source);
		...
		if($tile_goc < $tile_sauresize)
		{
			$new_pos_top = ($heightz/($widthz/$param_w)-$param_h)/2;
		}
		else
		{
			$new_pos_left = ($widthz/($heightz/$param_h)-$param_w)/2;
		}
		
		$cropped = $resized->crop($new_pos_left, $new_pos_top, $param_w, $param_h);
		$cropped->saveToFile($source.".".$tail.".jpg");
	}
}
