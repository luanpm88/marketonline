...
class Products extends PbModel {
	...
	function getMobileHomeCats($zone_id=38) {
		...
		foreach($industries as $key => $cat) {
			...
			$result = $this->findAll("c.picture as company_logo,m.membertype_id,Product.*,Product.name AS title,Product.content AS digest,c.shop_name, c.cache_spacename", $joins, $conditions, "Product.created DESC", 20);
			$result = $this->formatItems($result);
