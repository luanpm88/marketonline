...
class Areas extends PbModel {
	...
	{
		$result = '';
		$tmp = $this->read("name, parent_id, top_parentid", $id);
		//var_dump($tmp);
		$result .= $tmp["name"];
		//var_dump($tmp);
		if($tmp["parent_id"] != $tmp["top_parentid"])
		{
			$tmp = $this->read("name, parent_id, top_parentid", $tmp["parent_id"]);
			$result .= ", ".$tmp["name"];
			//var_dump($tmp["name"]);
			$tmp = $this->read("name, parent_id, top_parentid", $tmp["top_parentid"]);
			$result .= ", ".$tmp["name"];
			//var_dump($tmp["name"]);
		}
		else
	...
	function getAreaOptions($ids = null)
 	{
 		$_PB_CACHE['area'] = cache_read("area");
 		$this->typeOptions = '';
 		$this->dbstuff->setFetchMode(ADODB_FETCH_ASSOC);
 		$this->params['data'] = $this->dbstuff->GetArray("SELECT id,parent_id,name,level FROM ".$this->table_prefix."areas WHERE level=2 ORDER BY display_order ASC");
 		$tmp_arr = array();
 		foreach ($this->params['data'] as $key=>$val) {
 			$tmp_arr[$val['id']] = $this->params['data'][$key];
 		}
 		...
 		foreach ($_PB_CACHE['area'][1] as $key=>$val) {
 			...
 			foreach ($_PB_CACHE['area'][2] as $key2=>$val2) {
 				if ($tmp_arr[$key2]['parent_id'] == $key) {
					...
					$selected = '';
					if(strpos("[0]".$ids, "[".$key2."]"))
					{
						$selected = ' selected="selected"';
					}
					
		 			$this->typeOptions.='<option value="'.$key2.'" class="option-level1"'.$selected.'>';
		 			$this->typeOptions.=str_repeat('&nbsp;&nbsp;', 1) . $val2;
		 			$this->typeOptions.='</option>' . "\n";
		 			foreach ($_PB_CACHE['area'][3] as $key3=>$val3) {
		 				if ($tmp_arr[$key3]['parent_id'] == $key2) {
	...
	function getChildArea($id)
	{
		...
		$childs = $this->dbstuff->GetArray("SELECT id FROM ".$this->table_prefix."areas WHERE parent_id=".intval($id));
	...
	{
		$childs = $this->dbstuff->GetArray("SELECT id,name FROM ".$this->table_prefix."areas WHERE name LIKE '%".trim($name)."%'");
		//echo "SELECT id,name FROM ".$this->table_prefix."areas WHERE name LIKE '%".trim($name)."%'";
		if(!empty($childs))
		{
			return $childs[0];
		}
