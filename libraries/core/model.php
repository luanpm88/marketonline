...
class PbModel extends Overloadable
{
	...
	function save($posts, $action=null, $id=null, $tbname = null, $conditions = null, $if_check_word_ban = false)
	{
		...
		foreach ($keys as $colname) {
			...
			$record[$colname] = stripslashes($posts[$colname]);
		}
	...
	function read($fields = null, $id = null, $tables = null, $conditions = null, $joins = null)
	{
		$tmp_tablename = null;
		if ($id!==null) {
			$this->id = $id;
		...
		$sql = "SELECT ".$fields." FROM ".$tmp_tablename." ".$multi_table." WHERE ".$field."='".$id."'";
		if(!empty($conditions)){
			if (is_array($conditions)) {
...
//		}
	...
	function fields($name, $conditions = null, $order = null) {
		if ($conditions === null) {
			$conditions = array($this->name . '.' . $this->primaryKey => $this->id);
		}
		if (is_array($conditions)) {
		...
		$sql = "select ".trim($name)." from ".$this->getTable(true)." where ".$conditions.$order;
		//echo $sql;
//		if ($this->cache_sql) {
//			$return = $this->dbstuff->CacheGetOne($this->cache_sql, $sql);
//		}else{
...
//var_dump($return);
	...
	function findCount_left($joins = null, $conditions = null, $countfield = null, $table_alias = null)
	{
		if (!$this->page_secure) {
			//if (isset($_GET['total_record'])) {
			//	return intval($_GET['total_record']);
...
//		$condition	 	= null;
	...
	function clicked_($id, $additionalParams = true)
	{
		if($additionalParams){
	...
	function clicked($code, $trade)
	{
		...
		{
			$logs = json_decode($trade["clicked_logs"], true);
			//var_dump($logs);
			$exsit = false;
		...
		if($added && isset($trade["company_id"]))
		{
			...
			$company->clicked($trade["company_id"], true);
		}
