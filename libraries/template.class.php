...
//		$this->use_sub_dirs = false;
	...
	function flash($message_code, $url, $pause = 1, $extra = '', $mes = "") {
		...
		if (empty($message_code) || !$message_code || $message_code=="failed") {
			$image = $images[0];
			$page_title = $box_title = $message = L('action_failed', "msg", $extra);
			$style = $styles[0];
		}elseif($message_code=="success" or true===$message_code or strstr("success", $message_code)){
			$image = $images[1];
			$style = $styles[1];
			$page_title = $box_title = $message = L("success", "msg", $extra);
		}else{
			if(is_array($message_code))
			{
				$image = $images[2];
				$style = null;
				$message = L($message_code["message"], "msg", $extra);
				$box_title = $message_code["box_title"];
				$page_title = $message_code["page_title"];
			}
			else
			{
				$image = $images[2];
		...
		if(detectMobile()) {
			//echo $this->flash_layout.
			template("default/mobile/flash");
		} else {
